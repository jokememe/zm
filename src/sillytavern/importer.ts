...
export function importPreset(
  ...
  const name = resolvePresetDisplayName(data, opts?.fileName);
  const settings = normalizePresetSettings({ ...data });
  return {
    name,
...
export function exportPreset(preset: ChatPreset): Record<string, any> {
  return denormalizePresetForExport(
    { ...(preset.settings || {}) },
...
export async function importJsonFile<T>(): Promise<T | null> {
  const r = await importJsonFileWithName<T>();
  return r?.data ?? null;
}
