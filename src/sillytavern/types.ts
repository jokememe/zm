...
#世界设定
[设定名]|类型：…|详细说明：…
--></Memory>                 ← 可选；有关键人物/物品/设定变化时增量更新（主键合并，只写变更字段）
说明：气数与名册/外交/城池由系统在回合结束后自动分析并改档，不要用 <vars> 改档。正文与 <sum> 中若发生收徒、离宗、交恶、结盟、纳贡、资源增减等，须写出具体人名/势力名与结果，供自动结算抽取。表格记忆与 <sum> 并存，勿省略 <sum>。`;
